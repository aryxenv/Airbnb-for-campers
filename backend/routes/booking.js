...
router.post("/update/dates", async (req, res, next) => {
  ...
  if (checkin && checkout) {
    ...
    if (checkinDate > checkoutDate) {
      return res
        .status(400)
        .json({ error: "Check out date must be bigger than check in date." });
    }
  }
  ...
  // check availability
  let dates_available = await checkPropertyAvailability(
  ...
  try {
    if (!dates_available) {
      return res
        .status(400)
        .json({ success: false, error: "Dates not available." });
    }
  ...
  try {
    ...
    if (guests) {
      if (capacity < guests) {
        return res
          .status(400)
          .json({ success: false, error: "Too many guests" });
      }

      return res
        .status(200)
        .json({ success: true, message: "Within guest limit" });
    }

    return res
      .status(400)
      .json({ success: false, error: "Property capacity not found." });
  } catch (error) {
    // unexpected error, log error
    console.log(error);
    return res
...
router.post("/pay", isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `https://aryxenv.dev/airbnb-camping/book?property_id=${property_id}&checkin=${checkin}&checkout=${checkout}&guests=${guests}&success=true`, //success page (booking confirmed page)
      cancel_url: `https://aryxenv.dev/airbnb-camping/property?property_id=${property_id}&checkin=${checkin}&checkout=${checkout}&guests=${guests}`, // canceled page (back to propertydetails)
    });
